...
def sendmailoforderdetailsAdmin(email_to='',OrderId='',subtotal='',ProductName='',Address='',Weight='',Quantity='',Price='',storeaddress='',CustomerName='',Store_Name='',IdCard=''):
    ...
    email_message.attach(MIMEText(html2.format(OrderId=OrderId,subtotal=subtotal,ProductName=a,Address=Address,Quantity="<br><hr>".join([str(i) for i in Quantity]),Price="<br>".join([str(i) for i in Price]),storeaddress=storeaddress,IdCard=IdCard,date_str=date_str,CustomerName=CustomerName), "html"))
    email_string = email_message.as_string()
    context = ssl.create_default_context()
    with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as server:
        server.login(email_from, password)
